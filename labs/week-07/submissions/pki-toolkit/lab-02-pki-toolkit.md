...
openssl x509 -in leaf_cert.pem -text -noout | grep -A4 "Authority Information Access"


# Convert PEM to DER format
openssl x509 -in leaf_cert.pem -outform DER -out cert.der
...
echo | openssl s_client -connect github.com:443 2>/dev/null | openssl x509 -out leaf_cert.pem

# Retrieve from expired endpoint
...
**What it does:**
Generates a Certificate Signing Request (CSR) that includes a public key and identity information (Subject DN). In simple terms, it prepares a request that asks a Certificate Authority (CA) to issue a certificate for you, while proving that you own the private key.
...
```bash
openssl req -new -x509 -key test\\\_key.pem -out test\\\_cert.pem -days 365 -subj "/CN=Week4-Lab-Test/O=CVI/C=US"
```
**What the output tells you:**
...
openssl ocsp \
 ```bash
openssl ocsp \\\\
  -issuer issuer\\\_cert.pem \\\\
  -cert leaf\\\_cert.pem \\\\
  -url "$OCSP\\\_URL" \\\\
  -resp\\\_text \\\\
  -noverify \\\\
  2>/dev/null | tee ocsp\\\_response.txt
...
```bash
# Create PFX bundle
openssl pkcs12 -export -out test\\\_bundle.pfx -inkey test\\\_key.pem -in test\\\_cert.pem

# Verify PFX bundle
openssl pkcs12 -info -in test\\\_bundle.pfx -noout
```
...
**What it does:**
Generates an RSA private key of a specified bit length. It creates a secure “secret key” used for encryption or proving identity. This private key is the foundation of asymmetric cryptography and must be kept protected.
...
### openssl pkey (Modern Method)

**What it does:**
Extracts the public key component from a private key file. In simple terms, it creates a version of the key that can be safely shared, while keeping the private key secure. It is the modern alternative to openssl rsa -pubout.

**When to use it:**
When extracting public keys from private keys, especially when working with keys generated using openssl genpkey.

Example command from my labs:

```bash
openssl pkey -in private\\\_key.pem -pubout -out public\\\_key.pem
```

**What the output tells you:**
Outputs the public key that corresponds to the private key. This public key can be used for signature verification or encryption workflows.

**Phase 1 source:** Week 2 — Cryptography Fundamentals (Digital Signatures)


### openssl genpkey — Generate Private Keys (Modern Method)

**What it does:**
Generates private keys using modern algorithms and configurable options. It is a more flexible and up-to-date way to create private keys compared to older commands like openssl genrsa.

**When to use it:**
When generating private keys with specific algorithm parameters, or when following current best practices for key generation.

Example command from my labs:

```bash
openssl genpkey -algorithm RSA -pkeyopt rsa\\\_keygen\\\_bits:2048 -out private\\\_key.pem
```

**What the output tells you:**
Produces a private key file in PEM format. The algorithm and key size are explicitly defined, giving more control over how the key is generated.
...
# Create signature
openssl dgst -sha256 -sign private\\\_key.pem -out artifact.sig artifact.txt

# Verify signature
openssl dgst -sha256 -verify public\\\_key.pem -signature artifact.sig artifact.txt
```
...
**What it does:**
Sends HTTP or HTTPS requests to a server and displays the response headers. It lets you “ask” a website how it is configured and what technologies or security settings it is using, for example, used to inspect server behavior, check for security headers, and identify CDN or load balancer indicators.
...
curl -sI https://bankofamerica.com | grep -i "server\\\\|cf-ray\\\\|x-cache\\\\|via\\\\|x-amz"
...
curl -s "https://\\\[CA-Issuers-URL]" -o intermediate.der
```
...
```bash
diff plaintext.txt plaintext\\\_decrypted.txt
```
...
### SSL Labs SSL Server Test (External TLS Configuration Analysis)
...
Example usage from my labs:
Go to https://crt.sh
 → Search for "bankofamerica.com" → Review historical issuances and issuer patterns
